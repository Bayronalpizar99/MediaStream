import 'dotenv/config';
import express from 'express';
import morgan from 'morgan';
import { randomUUID } from 'node:crypto';
import os from 'node:os';
import dbConfig from '../config';
import {
  COLLECTIONS_NAMES,
  HttpErrorStatusCodes,
} from '../constants';
import { MediaFile } from '../models/MediaModel';

const app = express();
app.use(express.json());
app.use(morgan('dev'));

const PORT = Number(process.env.STREAMING_NODE_PORT ?? 4002);
const NODE_ID = process.env.STREAMING_NODE_ID ?? randomUUID();
const NODE_NAME = process.env.STREAMING_NODE_NAME ?? 'streaming-node';
const COORDINATOR_URL =
  process.env.COORDINATOR_URL ?? 'http://localhost:3000';
const SHARED_SECRET = process.env.NODE_SHARED_SECRET;
let activeStreams = 0;

const ensureAuthorized = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  if (!SHARED_SECRET) {
    return next();
  }
  if (req.path === '/health') {
    return next();
  }
  const headerSecret = req.headers['x-node-secret'];
  if (headerSecret !== SHARED_SECRET) {
    return res
      .status(HttpErrorStatusCodes.UNAUTHORIZED)
      .send({ message: 'Unauthorized node request' });
  }
  return next();
};

app.use(ensureAuthorized);

const registerWithCoordinator = async () => {
  try {
    await fetch(`${COORDINATOR_URL}/nodes/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id: NODE_ID,
        name: NODE_NAME,
        role: 'streaming',
        baseUrl: `http://localhost:${PORT}`,
        location: process.env.NODE_LOCATION ?? 'local',
      }),
    });
    console.log(`[${NODE_NAME}] Registered with coordinator`);
  } catch (error) {
    console.error(`[${NODE_NAME}] Failed to register node:`, error);
  }
};

const sendHeartbeat = async () => {
  try {
    await fetch(`${COORDINATOR_URL}/nodes/heartbeat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id: NODE_ID,
        metrics: {
          cpu: Math.min(
            100,
            (os.loadavg()[0] / os.cpus().length) * 100,
          ),
          ram: Math.min(
            100,
            (process.memoryUsage().rss / os.totalmem()) * 100,
          ),
          uptimeSeconds: process.uptime(),
          tasks: activeStreams,
        },
      }),
    });
  } catch (error) {
    console.error(`[${NODE_NAME}] Heartbeat error:`, error);
  }
};

const getMediaFile = async (fileId: string) => {
  const fileRef = dbConfig.db.collection(COLLECTIONS_NAMES.MEDIA_FILES).doc(fileId);
  const fileDoc = await fileRef.get();
  if (!fileDoc.exists) {
    return null;
  }
  return { id: fileDoc.id, ...(fileDoc.data() as MediaFile) };
};

app.post('/streams/:fileId', async (req, res) => {
  try {
    const { fileId } = req.params;
    const mediaFile = await getMediaFile(fileId);
    if (!mediaFile) {
      return res
        .status(HttpErrorStatusCodes.NOT_FOUND)
        .send({ message: 'File not found' });
    }

    const bucketFile = dbConfig.storage.file(mediaFile.storagePath);
    const [metadata] = await bucketFile.getMetadata();

    res.setHeader('Content-Disposition', `attachment; filename="${mediaFile.filename}"`);
    res.setHeader('Content-Type', metadata.contentType || mediaFile.contentType || 'application/octet-stream');
    if (metadata.size) {
      res.setHeader('Content-Length', metadata.size);
    }

    activeStreams += 1;
    bucketFile.createReadStream().on('end', () => {
      activeStreams = Math.max(0, activeStreams - 1);
    }).on('error', () => {
      activeStreams = Math.max(0, activeStreams - 1);
    }).pipe(res);
  } catch (error) {
    console.error(`[${NODE_NAME}] streaming error:`, error);
    activeStreams = Math.max(0, activeStreams - 1);
    return res
      .status(HttpErrorStatusCodes.INTERNAL_SERVER_ERROR)
      .send({ message: 'Stream failed' });
  }
});

app.get('/health', (_req, res) => {
  res.send({
    nodeId: NODE_ID,
    name: NODE_NAME,
    role: 'streaming',
    status: 'online',
    uptimeSeconds: process.uptime(),
  });
});

app.listen(PORT, async () => {
  console.log(`[${NODE_NAME}] Listening on http://localhost:${PORT}`);
  await registerWithCoordinator();
  await sendHeartbeat();
  setInterval(sendHeartbeat, 5_000);
});
